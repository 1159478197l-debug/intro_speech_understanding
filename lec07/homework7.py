...
def major_chord(f, Fs):
    ...
    majorthird = np.cos(np.power(G,4) * omegao*n)
...
def spectral_analysis(x, Fs):
    ...
    k1 = np.argmac(X[:int(N/2)])
    X[k1] = 0
    k2 = np.argmac(X[:int(N/2)])
    X[k2] = 0
    k3 = np.argmac(X[:int(N/2)])
    frequencies = sorted([k1,k2,k3])*(Fs/N)
    return frequencies[0],frequencies[1],frequeuncies[2]
