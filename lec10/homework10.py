...
def get_features(waveform, Fs):
    ...
    frameends = [ m for m in range(1, len(energy)) if VAD[m-1]==1 and VAD [m]==ø|
...
def train_neuralnet(features, labels, iterations):
    ...
    model = torch.nn.Sequential(torch.nn.LayerNorm(NFEATS, dtype=float), torch.nn.Linear(NFEATS, NLABELS,
